...
from app.services.llm.mimo import MiMoLLM
...
def create_llm(provider: str | None = None) -> BaseLLM:
    ...
    Args:
        provider: "qwen" | "openai" | "glm" | "mimo" | "mock"，默认从配置读取
    ...
    if provider != "mock":
        api_key_attr = "MIMO_API_KEY" if provider == "mimo" else f"{provider.upper()}_API_KEY"
